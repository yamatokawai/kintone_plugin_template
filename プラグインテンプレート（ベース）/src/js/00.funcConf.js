  ...
  window.SR.funcConf = {
      ...
      for (let elem of layouts) {
        ...
        switch (elem.type) {
          ...
          case 'SUBTABLE': {
            ...
            elem.fields.map((el) => {
              if (FIELD.appField.filter !== 'all' || !FIELD.appField.type.includes(el.type)) return;
              fieldArr.push({
    ...
    createInitialFields: async () => {
      ...
      Object.keys(FIELDS_CONF).forEach((el) => {
        ...
        if (FIELDS_CONF[el].type.includes('dropdown') || FIELDS_CONF[el].type.includes('multiChoice'))
          fieldsItems.unshift({ label: '-----', value: 'default' });
      });
      let setValArr = [];
