  ...
  if (items.length) {
    ...
    const renderFN = (dataCell) => {
      ...
      dropdown.onchange = (event) => {
        const aaa = document.getElementsByClassName(dropdown.className);
        let bbb = [];
        for (let aa of aaa) {
          aa.error = "";
          if (aa !== event.currentTarget && aa.value === event.detail.value) bbb.push(aa);
        }
        for (let bb of bbb) bb.error = "重複してるよ";
      };
      return dropdown;
    };
